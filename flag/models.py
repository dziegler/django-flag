...
class FlaggedContent(models.Model):
    ...
    creator = models.ForeignKey(User, blank=True, null=True, related_name="flagged_content") # user who created flagged content -- this is kept in model so it outlives content
    ...
    def __unicode__(self):
        try:
            return self.content_type.__unicode__()+" object: "+self.content_object.__unicode__() \
            + " - flagged by " + self.creator.__unicode__()
        except:
            return self.content_type.__unicode__()+" object: "+self.content_object.__unicode__() \
...
class FlagInstance(models.Model):
    ...
    user = models.ForeignKey(User, blank=True, null=True) # user flagging the content
...
def add_flag(flagger, content_type, object_id, content_creator, comment):
    ...
    try:
        ...
        if content_creator and content_creator.is_authenticated():
            flagged_content = FlaggedContent(content_type=content_type, object_id=object_id, creator=content_creator)
        else:
            flagged_content = FlaggedContent(content_type=content_type, object_id=object_id)
        flagged_content.save()
    if flagger.is_authenticated():
        flag_instance = FlagInstance(flagged_content=flagged_content, user=flagger, comment=comment)
    else:
        flag_instance = FlagInstance(flagged_content=flagged_content, comment=comment)
    flag_instance.save()
